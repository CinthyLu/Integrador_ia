...
* **Entidad y Repositorio:** `CategoryEntity` mapea la tabla `categories`. `CategoryRepository` extiende `JpaRepository`.
* **Controlador Seguro:** Protegemos las operaciones de escritura en [CategoryController](file:///c:/Users/MSI/Desktop/PPW/Proyecto%20final/academic-events-api/src/main/java/ec/edu/ups/icc/events/categories/controllers/CategoryController.java) mediante el uso de `@PreAuthorize("hasRole('ADMIN')")`:
  ```java
  @PostMapping
  ...
  ```
  Los endpoints `GET` son públicos para cualquier usuario autenticado de acuerdo al contexto general.

### 6.2 Búsqueda, Filtrado y Paginación de Eventos
* **Búsqueda Dinámica con Specifications:** Para implementar filtros combinados y flexibles en `GET /api/events`, utilizamos `JpaSpecificationExecutor` en `EventRepository` y creamos un filtro en `EventService` usando `Specification<EventEntity>`:
  * Filtro por texto (título, descripción o locación, insensible a mayúsculas/minúsculas).
  * Filtro exacto por categoría (`categoryId`) y modalidad (`modality`).
  * Rango de fechas (`startDate` y `endDate`).
* **Paginación:** El controlador acepta `page` y `size` devolviendo un objeto paginado `Page<EventDTO>` a través de `Pageable` y `PageRequest.of(page, size)`.

### 6.3 Verificación de Propiedad (Ownership) y Corrección de AOP Bypass
> [!IMPORTANT]
> **El problema del proxy AOP de Spring:** Si se anota un método dentro de la misma clase del servicio (como `updateEventWithOwnership` llamado desde `updateEvent` en `EventService`) con `@PreAuthorize`, la seguridad se **salta por completo** debido al comportamiento de self-invocation de los proxies de Spring AOP. Además, la expresión `#event.organizer.id == authentication.principal.id` fallará ya que el `principal` cargado de tipo `org.springframework.security.core.userdetails.User` no expone la propiedad `id`.
> 
> **Solución implementada:** Se reemplazó la validación por una verificación programática robusta en [EventService](file:///c:/Users/MSI/Desktop/PPW/Proyecto%20final/academic-events-api/src/main/java/ec/edu/ups/icc/events/events/services/EventService.java):
  ```java
  ...
  ```
  Este método helper se ejecuta directamente al inicio de `updateEvent` y `deleteEvent`.

### 6.4 Módulo de Sesiones (CRUD Completo con Ownership)
Las sesiones representan sub-actividades de un evento (por ejemplo, ponencias de un congreso). La base de datos asocia cada sesión a un `event_id` mediante una relación Many-to-One.

#### 1. DTO de Sesión ([SessionDTO.java](file:///c:/Users/MSI/Desktop/PPW/Proyecto%20final/academic-events-api/src/main/java/ec/edu/ups/icc/events/sessions/dtos/SessionDTO.java))
```java
...
#### 2. Repositorio ([SessionRepository.java](file:///c:/Users/MSI/Desktop/PPW/Proyecto%20final/academic-events-api/src/main/java/ec/edu/ups/icc/events/sessions/repositories/SessionRepository.java))
Permite listar las sesiones asignadas a un evento en particular:
```java
@Repository
...
#### 3. Servicio de Negocio ([SessionService.java](file:///c:/Users/MSI/Desktop/PPW/Proyecto%20final/academic-events-api/src/main/java/ec/edu/ups/icc/events/sessions/services/SessionService.java))
Implementa la lógica del CRUD de sesiones. Para la creación, modificación y eliminación, verifica programáticamente que la fecha de fin de la sesión sea posterior a su fecha de inicio y valida que el usuario autenticado sea `ADMIN` o el `organizer` del evento principal al cual pertenece la sesión:
```java
@Service
@Transactional
public class SessionService {
    ...
    public SessionDTO createSession(Long eventId, SessionDTO dto) {
        ...
        verifyEventOrganizer(event); // Valida propiedad
        // ... validaciones de fecha y guardado
    }
...
#### 4. Controlador ([SessionController.java](file:///c:/Users/MSI/Desktop/PPW/Proyecto%20final/academic-events-api/src/main/java/ec/edu/ups/icc/events/sessions/controllers/SessionController.java))
Expone el CRUD de sesiones bajo las rutas `/api/events/{eventId}/sessions` y `/api/sessions/{id}`. Protege los endpoints de escritura con validaciones de rol previas antes de delegar a la validación de propiedad en el servicio:
```java
@RestController
@RequestMapping("/api")
